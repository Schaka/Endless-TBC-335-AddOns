...
local function getDefaultCooldown()
	...
	for k, class in pairs(Gladdy.CLASSES) do
...
function Cooldowns:IconsSetPoint(button)
	...
	tbl_sort(orderedIcons, function(a, b)
		...
		local spellTimeA = Gladdy.db.cooldownCooldownsOrder[button.class][tostring(a.spellId)]
		local spellTimeB = Gladdy.db.cooldownCooldownsOrder[button.class][tostring(b.spellId)]

		return spellTimeA < spellTimeB
	end)
...
function Cooldowns:UpdateCooldowns(button)
	...
	for k, v in pairs(Gladdy:GetCooldownList()[race]) do
		if Gladdy.db.cooldownCooldowns[tostring(k)] and k == race then
			if (type(v) ~= "table" or (type(v) == "table" and v.class == nil)) then
				Cooldowns:AddCooldown(k, v, button)
			end
			if (type(v) == "table" and v.class ~= nil and v.class == class) then
				Cooldowns:AddCooldown(k, v, button)
			end
		end
	end
		...
		for spellId,cooldown in pairs(Gladdy:GetCooldownList()[race]) do
			...
			group[class].args[tostring(spellId)] = {
				...
				args = {
					toggle = {
						type = "toggle",
						name = select(1, GetSpellInfo(spellId)) .. (type(cooldown) == "table" and race and (" - " .. race) or ""),
						order = 1,
						width = 1.1,
						image = select(3, GetSpellInfo(spellId)),
						get = function()
