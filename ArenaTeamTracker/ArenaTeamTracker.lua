    ...
    icon.Start = function(sentCD, nextcharge)
        ...
        if icon.maxcharges then
            ...
            if charges == 2 or nextcharge == icon.maxcharges then
                CooldownFrame_SetTimer(cd, GetTime(), icon.cooldown, 1, 1, 1)
            ...
            elseif charges == 1 and nextcharge == 1 and icon.starttime < GetTime() then
                CooldownFrame_SetTimer(cd, icon.starttime, icon.cooldown, 1)
...
function ATT:ToggleAnchorDisplay()
    ...
    for k = GetNumPartyMembers() + 1, 4 do
...
function ATT:COMBAT_LOG_EVENT_UNFILTERED(event, ...)
    if not self:IsShown() then
        return
    end
...
function ATT:RequestSync()
    if self.useCrossAddonCommunication and IsInGroup() then
        SendAddonMessage(ChatPrefix, "Version|" .. ATTversion, IsInGroup(2) and "INSTANCE_CHAT" or IsInGroup(1) and "RAID")
    end
end
...
function ATT:FindAbilityIcon(ability, id)
    ...
    if id then
        ...
        _, _, icon = GetSpellInfo(self:FindAbilityID(ability))
    end
    return icon
end
...
function ATT:CreateOptions()
    ...
    growLeft:SetPoint("TOP", panel, "TOP", -0, -45)
...
function ATT:CreateAbilityEditor()
    ...
    scrollframe:SetPoint('LEFT', 16, -120)
    child.dropdown2 = nil
